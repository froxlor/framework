...
SH,
    ...
    private function packages(array $packages): array
    {
        return [
            ...
            'done',
            'if [ "${#missing[@]}" != 0 ]; then',
            // Never replace an administrator's start policy.
            '  [ ! -e /usr/sbin/policy-rc.d ] && [ ! -L /usr/sbin/policy-rc.d ] || exit 1',
            ...
            '  apt-get -o DPkg::Lock::Timeout=120 --no-install-recommends --no-upgrade --no-remove -y install "${missing[@]}"',
