...
namespace Froxlor\Core\Http\Controllers\Api\Tenant\Plan;
...
class PlanResourceController extends Controller
{
    ...
    {
        Gate::authorize('tenantResourceViewAny', [$plan, $tenant]);
    ...
    /**
     * Assign or update a resource limit on a tenant-owned plan.
     */
    public function store(Request $request, Tenant $tenant, Plan $plan)
    {
        Gate::authorize('tenantResourceCreate', [$plan, $tenant]);
        ...
        Audit::notice('resource "' . $resource->key . '" assigned to plan "' . $plan->name . '"', $tenant, context: [
            'plan_id' => $plan->id,
    ...
    /**
     * Remove a resource from a tenant-owned plan.
     */
    public function destroy(Tenant $tenant, Plan $plan, Resource $resource)
    {
        Gate::authorize('tenantResourceDelete', [$plan, $tenant, $resource]);
